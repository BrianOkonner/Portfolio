...
IntStack():top(0){};//Контрутор инициализирует индекс в 0 (стек пуст) верхушка показывает сколько элементов в стеке
	...
	public:
		...
		{
			os<<it.current()<<" ";//выводит элемент стека на который указывает итератор через пробел 
...
int main()
{
	...
	//вызывается сначала -- а потом не пергруженный cout т.к (я не успею написать но я знаю что происходит)
