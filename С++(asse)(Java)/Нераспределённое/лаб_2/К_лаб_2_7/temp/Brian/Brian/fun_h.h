...
void mass_cel_rand4N(int *f, int n);//Заполнить массив целых случайным образом (1-1000 числа)
...
void matrB9L3(int **a,int n);//Создание матрицы вида например при n=4 :(1)1 2 3 4(2)12 1 2 5(3)11 4 3 6(4)10 9 8 7.
void matrB7L3(int **a,int n,int m);//Создание матрицы каждый элемент которой есть сумма номеров строки и столбца в которых он стоит
int *mass_dclr(int *a,int n);//Объявление динамического массива(Инициализируйте указатель на матрицу (например int **a=1)или вылезет предупреждение)
...
//Версия функции seekg() с двумя параметрами перемещает указатель ввода на offset байт от позиции, заданной параметром origin. Параметр offset имеет тип streamoff, который определен в заголовке iostream.h. Объект типа streamoff способен содержать самое большое допустимое значение, которое может иметь параметр offset.Параметр origin имеет тип ios::seek_dir и представляет собой перечисление, которое имеет следующие значения.
