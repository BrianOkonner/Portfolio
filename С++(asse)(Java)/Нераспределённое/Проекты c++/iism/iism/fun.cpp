...
int maxof2(int a,int b)//Максимум из 2 чисел
{
	if (a<=b) return b;
	else return a;
}
double drobmod(double a,unsigned int b)//Остаток от деления для дробных чисел
{
	double c;
	int d;
	d=int(a/b);
...
/*void krvvod(ifstream in,char *str)
{
	for(int i=0;str[i+1]=' '&&str[i+2]=' ';i++)
		in>>str[i];
}*/
...
{   int n,n2;
    ...
    n=strlen(S2);         //Длина подстроки
	  if (strlen(S1)>=num) 
	  {
		...
		assemStrcat(s2,S2);    //вставляем слово
		 assemStrcat(s2," " );
		assemStrcat(s2,s3);     
