...
{  // имя нового процесса с пробелом 
	char lpszComLine[80]="C:\\ConsoleProcess.exe ";  // для символьного представления дескриптора 
	char lpszHandle[20]; 
 ...
 if(!SetHandleInformation(    hThread,    // дескриптор потока 
 ...
 si.cb=sizeof(STARTUPINFO);  // преобразуем дескриптор в символьную строку
 _itoa((int)hThread,lpszHandle,10);  // создаем командную строку
 strcat(lpszComLine,lpszHandle);  // запускаем новый консольный процесс 
