// Пример процесса, который передает наследуемый дескриптор потока // дочернему процессу через командную строку.  
...
void thread() {  for ( ; ; )  {   count++;   Sleep(500);   _cprintf ("count = %d\n", count);  } } 
 
int main() {  char lpszComLine[80]; // для командной строки 
 ...
 sa.nLength = sizeof(SECURITY_ATTRIBUTES);  sa.lpSecurityDescriptor = NULL; 
 // защита по умолчанию 
 sa.bInheritHandle = TRUE;  // дескриптор потока наследуемый    // запускаем поток-счетчик
 hThread = CreateThread(&sa, 0, (LPTHREAD_START_ROUTINE)thread, NULL, 0, &IDThread);  if (hThread == NULL)   return GetLastError(); 
 
 // устанавливаем атрибуты нового процесса
 ZeroMemory(&si, sizeof(STARTUPINFO));  si.cb=sizeof(STARTUPINFO);  // формируем командную строку
 wsprintf(lpszComLine, "C:\\ConsoleProcess.exe %d", (int)hThread);   // запускаем новый консольный процесс
 cout<<lpszComLine;
 system("pause");
 ...
 _cputs("Press any key to exit.\n");  _getch();  // закрываем дескриптор потока 
 CloseHandle(hThread); 
