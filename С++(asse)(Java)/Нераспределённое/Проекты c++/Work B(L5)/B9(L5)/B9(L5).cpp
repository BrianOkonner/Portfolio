...
//  работа с функцикй strlen(string)
//       пример
...
// нумерация в массиве начинаеться с 0 заканч-ся strlen-1
// [0] - с 
// [5] или [strlen-1]-  а 
// [strlen] /0  конец строки
					   ...
					   if (iCTWords> 0)
					   {
						...
						if (iCTWords > 1)
						  ...
						  for (i = iCTWords - 1; i>=0; i--)
							   ...
							   if (strlen(ARRWORD[i]) < strlen(ARRWORD[i - 1]))
							   {  // Меняем местами
