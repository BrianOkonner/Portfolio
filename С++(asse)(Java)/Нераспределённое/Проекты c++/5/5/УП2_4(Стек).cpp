...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance,
     ...
     hwnd = CreateWindow (szAppName, "Stack",
                          WS_OVERLAPPEDWINDOW,
...
CreateWindow("button", "Top", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
     {static Stack A;
 static int t=5,x=600,y=20,a=1,size_x,size_y;
	...
	//{}
	  ...
	  if (LOWORD(wParam)==10002)
	  {
		  if (!A.isEmpty())
		  {
		   ...
		   {
		      MessageBox(hwnd,"Jxthtl Пуст!","Ошибка",MB_OK )  ;             // t=0;
		                       // InvalidateRect(hwnd,NULL,FALSE);	
		   }
	 ...
	 case WM_PAINT:  
		  ...
		  strcpy(s1,"\0");
	     switch (t)
	     {//case 0: TextOut(hdc,x,y,"Стек Пустой",20) ;     
	      //break;

		  case 1:

			  strcat(s1,"Добавили в Стек ");
              strcat(s1,s);
		...
		TextOut(hdc,x,y,s1,strlen(s1)) ;
		  ...
		  case 3:  
			strcat(s1,"Значение на верхушке Стека ");
            strcat(s1,s);
            TextOut(hdc,x,y,s1,strlen(s1)) ;
			break;
