...
inline void  PrintNode(ostream& _out, const Tree* node,int p);
...
int main(void)
{
	...
	catch(invalid_argument)
	{
		...
		try
		{
            ...
            {
                outq<<"YES"<<endl;
                PrintNode(outq, tree,0);
                outq <<endl;
                PrintNode(outq, tree2,0);
                outq <<endl;
                return 0;
            }
		}
...
inline void PrintNode(ostream& _out, const Tree* node,int p)
{
    if(node != NULL)
	{
        ...
        PrintNode(_out, node->left,p);
        PrintNode(_out, node->right,p);
    }
}
inline void WalkZNode(Tree* node)
{
