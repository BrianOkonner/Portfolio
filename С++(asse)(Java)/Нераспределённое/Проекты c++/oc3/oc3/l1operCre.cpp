...
DWORD WINAPI threadFunc(LPVOID param)
{
	...
	while (!endWork) {
		int random_index =( rand() / parameters->n)% parameters->n;//Всмысле, это по модулю размерности массива а не "на"...(По моему тогда так надо).
		EnterCriticalSection(&csArray);
		if (parameters->mas[random_index] == 0)
		{
			Sleep(5);
			parameters->mas[random_index] = parameters->index;
			LeaveCriticalSection(&csArray);
			editsCount++;
			Sleep(5);
		}
		...
		{
			cout << "\nMy number " << parameters->index<< " marked elements total " << editsCount << " element,that can`t be marked " << random_index + 1 << " index\n";
			LeaveCriticalSection(&csArray);
...
int main() 
{
	...
	for (int i = 0; i < number_of_markers; i++) 
	{
		...
		parameters[i].index = 19;//Мой номер же 19(не в вашей группе, Владислав.)?
